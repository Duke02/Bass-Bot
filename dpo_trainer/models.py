...
class AudioDiffusion(nn.Module):
    ...
    def forward(self, latents, prompt, validation_mode=False):
        ...
        if validation_mode:
            timesteps = (self.noise_scheduler.num_train_timesteps // 2) * torch.ones((bsz,), dtype=torch.int64,
                                                                                     device=device)
        else:
        ...
        if self.set_from == "random":
            ...
            compressed_latents = self.group_in(noisy_latents.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1,
                                                                                                       2).contiguous()
            model_pred = self.unet(
...
class DPOAudioDiffusion(AudioDiffusion):
    ...
    def __init__(self, text_encoder_name,
        ...
        super().__init__(text_encoder_name, scheduler_name, unet_model_name, unet_model_config_path, snr_gamma,
                         freeze_text_encoder, uncondition)
    ...
    def forward(self, latents, prompt, ref_unet, validation_mode=False, sft=False):
        ...
        if sft:
            ...
            if self.set_from == "random":
                ...
                model_pred = self.group_out(model_pred.permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1,
                                                                                                 2).contiguous()
            ...
            if validation_mode:
                timesteps = (self.noise_scheduler.num_train_timesteps) * torch.ones((bsz,), dtype=torch.int64,
                                                                                    device=device).repeat(2)
            else:
                # Sample a random timestep for each instance
