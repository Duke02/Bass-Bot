# This all came from Tango2
# GitHub: https://github.com/declare-lab/tango/blob/master/tango2/tango2-train.py
# Arxiv Paper - https://arxiv.org/abs/2404.09956
...
from models import build_pretrained_models, DPOAudioDiffusion

logger = get_logger(__name__)


class Arguments(BaseModel):
    train_file: str = Field("data/train_audiocaps.json", description="A csv or a json file containing the training data.")
    validation_file: str = Field("data/valid_audiocaps.json", description="A csv or a json file containing the validation data.")
    test_file: str = Field("data/test_audiocaps_subset.json", description="A csv or a json file containing the test data for generation.")
    num_examples: int = Field(-1, description="How many examples to use for training and validation.")
    text_encoder_name: str = Field("google/flan-t5-large", description="Text encoder identifier from huggingface.co/models.")
    scheduler_name: str = Field("stabilityai/stable-diffusion-2-1", description="Scheduler identifier.")
    unet_model_name: Optional[str] = Field(None, description="UNet model identifier from huggingface.co/models.")
    unet_model_config: Optional[str] = Field(None, description="UNet model config json path.")
    beta_dpo: float = Field(5000, description="Beta value for DPO.")
    hf_model: Optional[str] = Field(None, description="Tango model identifier from huggingface: declare-lab/tango")
    snr_gamma: Optional[float] = Field(None, description="SNR weighting gamma for loss rebalancing (Recommended: 5.0).")
    freeze_text_encoder: bool = Field(False, description="Freeze the text encoder model.")
    text_column: str = Field("captions", description="Column name for input texts in datasets.")
    audio_column: str = Field("location", description="Column name for audio paths in datasets.")
    audio_w_column: str = Field("chosen", description="Column name for winner audio paths in dpo dataset.")
    audio_l_column: str = Field("rejected", description="Column name for loser audio paths in dpo dataset.")
    text_column_dpo: str = Field("prompt", description="Column name for input texts in dpo dataset.")
    dpo_beta: float = Field(2000, description="Beta value for dpo.")
    uncondition: bool = Field(False, description="10% uncondition for training.")
    prefix: Optional[str] = Field(None, description="Add prefix in text prompts.")
    per_device_train_batch_size: int = Field(2, description="Batch size per device for training.")
    per_device_eval_batch_size: int = Field(2, description="Batch size per device for validation.")
    learning_rate: float = Field(3e-8, description="Initial learning rate.")
    weight_decay: float = Field(1e-8, description="Weight decay to use.")
    num_train_epochs: int = Field(40, description="Total number of training epochs.")
    max_train_steps: Optional[int] = Field(None, description="Total training steps, overrides num_train_epochs if provided.")
    gradient_accumulation_steps: int = Field(4, description="Steps to accumulate before update.")
    lr_scheduler_type: str = Field("linear", description="The scheduler type to use.")
    num_warmup_steps: int = Field(0, description="Number of warmup steps in lr scheduler.")
    ...
    adam_epsilon: float = Field(1e-08, description="Epsilon value for Adam optimizer.")
    output_dir: Optional[str] = Field(None, description="Directory to store final model.")
    seed: Optional[int] = Field(None, description="Random seed for reproducibility.")
    checkpointing_steps: str = Field("best", description="Checkpointing strategy ('epoch' or 'best').")
    save_every: int = Field(5, description="Save model after every N epochs if checkpointing is 'best'.")
    resume_from_checkpoint: Optional[str] = Field(None, description="Resume training from checkpoint.")
    with_tracking: bool = Field(False, description="Enable experiment tracking.")
    sft_first_epochs: int = Field(0, description="Number of SFT epochs before preference training.")
    dataset_dir: str = Field("~/.cache/huggingface/datasets", description="Dataset storage location.")
    report_to: str = Field("all", description="Reporting integrations (tensorboard, wandb, comet_ml, clearml).")
...
class DPOText2AudioDataset(Dataset):
    def __init__(self, dataset, prefix, text_column, audio_w_column, audio_l_column, num_examples=-1):
        ...
        if num_examples != -1:
            self.inputs, self.audios_w, self.audios_l = self.inputs[:num_examples], self.audios_w[
                                                                                    :num_examples], self.audios_l[
                                                                                                    :num_examples]
            self.indices = self.indices[:num_examples]
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        for i in range(len(dataset['train'])):
            sf.write(f"{dataset_dir}/audio_alpaca/chosen_{i}.wav", dataset['train'][i]['chosen']['array'],
                     samplerate=16000)
            sf.write(f"{dataset_dir}/audio_alpaca/reject_{i}.wav", dataset['train'][i]['rejected']['array'],
                     samplerate=16000)
    ...
    model = DPOAudioDiffusion(
        args.text_encoder_name, args.scheduler_name, args.unet_model_name, args.unet_model_config, args.snr_gamma,
        args.freeze_text_encoder, args.uncondition, args.beta_dpo
    )
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = DPOText2AudioDataset(raw_dataset_train, prefix, 'prompt', 'chosen', 'rejected',
                                             args.num_examples)
        eval_dataset = Text2AudioDataset(raw_dataset_valid_test["validation"], prefix, text_column, audio_column,
                                         args.num_examples)
        test_dataset = Text2AudioDataset(raw_dataset_valid_test["test"], prefix, text_column, audio_column,
                                         args.num_examples)

        accelerator.print(
            "Num instances in train: {}, validation: {}, test: {}".format(train_dataset.get_num_instances(),
    ...
    if args.freeze_text_encoder:
        ...
        if args.unet_model_config:
            ...
            optimizer_parameters = list(model.unet.parameters()) + list(model.group_in.parameters()) + list(
                model.group_out.parameters())
    ...
    num_trainable_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad) + sum(
        p.numel() for p in ref_unet.parameters() if p.requires_grad)
